...
class LightGBM_RFE_FeatureSelector:
    """
    LightGBM Recursive Feature Elimination for feature selection with hyperparameter optimization
    """

    def __init__(self, n_estimators=n_estimators, n_folds=n_folds, random_state=random_state,
                 optimize_params=True, n_iter=10):
        """
        Initialize the LightGBM feature selector

        Parameters:
        -----------
        n_estimators : int
            Number of trees in LightGBM (will be overridden if optimize_params=True)
        n_folds : int
    ...
    def fit(self, X, y, verbose=True):
        """
        Perform the LightGBM-RFE feature selection process
        ...
        if verbose:
            print(f"Starting LightGBM-RFE feature selection with {n_features} features")
            print("=" * 60)
        ...
        while len(current_features) >= 1:
            ...
            for fold, (train_idx, val_idx) in enumerate(kf.split(X_current)):
                ...
                # Create LightGBM model with optimized or default parameters
                if self.best_params:
                ...
                y_pred = lgb_model.predict(X_fold_val)
                ...
                fold_importance.append(lgb_model.feature_importances_)
    ...
    def evaluate_final_model(self, X_train, X_test, y_train, y_test, feature_names=None,
                             save_shap=True, output_dir="lightgbm_rfe_results"):
        """
        Evaluate the final LightGBM model with SHAP analysis
        """
        if self.optimal_features is None:
            raise ValueError("Model has not been fitted yet. Call fit() first.")
        ...
        plt.ylabel('Feature')
        plt.title('LightGBM Feature Importance - Optimal Features')
    ...
    def save_results_to_csv(self, output_dir="lightgbm_rfe_results"):
...
def main():
    """Main execution function with lightgbm feature selection"""
    # File path - update with your actual file path
    file_path = '特征选择fc1.csv'
    lightgbm_output_dir = "lightgbm_rfe_results1"
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("lightgbm RFE 特征选择")
        print("=" * 80)

        # lightgbm RFE
        lightgbm_selector = LightGBM_RFE_FeatureSelector(
        ...
        lightgbm_selector.fit(X_train, y_train)
        catboost_optimal_features = lightgbm_selector.get_optimal_features()

        # Plot CatBoost results
        lightgbm_selector.plot_results()

        # Save CatBoost results
        lightgbm_selector.save_results_to_csv(lightgbm_output_dir)

        # CatBoost comprehensive evaluation with SHAP
        catboost_results = lightgbm_selector.evaluate_final_model(
        ...
        print("\n" + "=" * 80)
        print("lightgbm FEATURE SELECTION SUMMARY")
        print("=" * 80)
