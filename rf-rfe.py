...
class RF_RFE_FeatureSelector:
    """
    Random Forest Recursive Feature Elimination for feature selection with hyperparameter optimization.
    """

    def __init__(self, n_estimators=n_estimators, n_folds=n_folds, random_state=random_state,
                 optimize_params=True, n_iter=10):
        """
        Initialize the Random Forest feature selector.

        Parameters:
        -----------
        n_estimators : int
            Number of trees in the forest (will be overridden if optimize_params=True).
        n_folds : int
    ...
    def fit(self, X, y, verbose=True):
        """
        Perform the RF-RFE feature selection process.
        ...
        if verbose:
            print(f"Starting RF-RFE feature selection with {n_features} features")
            print("=" * 60)
        ...
        while len(current_features) >= 1:
            ...
            for fold, (train_idx, val_idx) in enumerate(kf.split(X_current)):
                ...
                # Create Random Forest model with optimized or default parameters
                if self.best_params:
            ...
            if len(current_features) == 1:
                feature_ranking[current_features[0]] = n_features # Assign highest rank to the last feature
                break
    ...
    def plot_results(self):
        ...
        ax1.scatter(optimal_n_features, optimal_mse, color='green', s=100, zorder=5, label='Optimal MSE')
        ax1_r2.scatter(optimal_n_features, optimal_r2, color='purple', s=100, zorder=5, label='Optimal R²')
    ...
    def evaluate_final_model(self, X_train, X_test, y_train, y_test, feature_names=None,
                             save_shap=True, output_dir="rf_rfe_results"):
        """
        Evaluate the final Random Forest model with SHAP analysis.
        """
        if self.optimal_features is None:
            raise ValueError("Model has not been fitted yet. Call fit() first.")
        ...
        plt.ylabel('Feature')
        plt.title('Random Forest Feature Importance - Optimal Features')
        plt.tight_layout()
        plt.show()
        ...
        if save_shap:
            ...
            try:
                ...
                shap.summary_plot(shap_values, X_train_optimal, plot_type="bar", show=False)
                plt.title("SHAP Feature Importance")
                plt.tight_layout()
                plt.show()

                # SHAP summary plot (beeswarm)
                shap.summary_plot(shap_values, X_train_optimal, show=False)
    ...
    def save_results_to_csv(self, output_dir="rf_rfe_results"):
        ...
        # Add feature lists
        feature_lists = [','.join(features) for features in self.results['features']]
...
def main():
    """Main execution function with Random Forest feature selection."""
    # File path - update with your actual file path
    file_path = '特征选择fc2.csv'
    rf_output_dir = "rf_rfe_results2"
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("RANDOM FOREST RFE 特征选择")
        print("=" * 80)

        # Random Forest RFE
        rf_selector = RF_RFE_FeatureSelector(
        ...
        rf_optimal_features = rf_selector.get_optimal_features()

        # Plot RF results
        rf_selector.plot_results()

        # Save RF results
        rf_selector.save_results_to_csv(rf_output_dir)

        # RF comprehensive evaluation with SHAP
        rf_results = rf_selector.evaluate_final_model(
        ...
        print("=" * 80)
        print(f"Optimal features ({len(rf_optimal_features)}): {rf_optimal_features}")
        print(f"Test R²: {rf_results['test_metrics']['r2']:.6f}")
        print(f"Test MSE: {rf_results['test_metrics']['rmse'] ** 2:.6f}")
        print(f"Test RMSE: {rf_results['test_metrics']['rmse']:.6f}")
        print(f"Test MAE: {rf_results['test_metrics']['mae']:.6f}")
        print(f"Test MAPE: {rf_results['test_metrics']['mape']:.6f}")
