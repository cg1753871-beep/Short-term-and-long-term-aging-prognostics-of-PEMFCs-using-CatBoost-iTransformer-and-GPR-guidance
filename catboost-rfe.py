...
class CatBoost_RFE_FeatureSelector:
    """
    CatBoost Recursive Feature Elimination for feature selection with hyperparameter optimization
    """

    def __init__(self, n_estimators=n_estimators, n_folds=n_folds, random_state=random_state,
                 optimize_params=True, n_iter=10):
        """
        Initialize the CatBoost feature selector

        Parameters:
        -----------
        n_estimators : int
            Number of trees in CatBoost (will be overridden if optimize_params=True)
        n_folds : int
    ...
    def _optimize_hyperparameters(self, X, y, verbose=True):
        ...
        # Create base model
        base_model = catboost.CatBoostRegressor(
    ...
    def fit(self, X, y, verbose=True):
        """
        Perform the CatBoost-RFE feature selection process
        ...
        if verbose:
            print(f"Starting CatBoost-RFE feature selection with {n_features} features")
            print("=" * 60)
        ...
        while len(current_features) >= 1:
            ...
            for fold, (train_idx, val_idx) in enumerate(kf.split(X_current)):
                ...
                # Create CatBoost model with optimized or default parameters
                if self.best_params:
                ...
                y_pred = catboost_model.predict(X_fold_val)
                ...
                fold_importance.append(catboost_model.feature_importances_)
    ...
    def evaluate_final_model(self, X_train, X_test, y_train, y_test, feature_names=None,
                             save_shap=True, output_dir="catboost_rfe_results"):
        """
        Evaluate the final CatBoost model with SHAP analysis
        """
        if self.optimal_features is None:
            raise ValueError("Model has not been fitted yet. Call fit() first.")
        ...
        plt.ylabel('Feature')
        plt.title('CatBoost Feature Importance - Optimal Features')
...
def main():
    """Main execution function with CatBoost feature selection"""
    # File path - update with your actual file path
    file_path = '特征选择fc1.csv'
    catboost_output_dir = "catboost_rfe_results1"
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("CATBOOST RFE 特征选择")
        print("=" * 80)

        # CatBoost RFE
        catboost_selector = CatBoost_RFE_FeatureSelector(
        ...
        catboost_selector.fit(X_train, y_train)
        catboost_optimal_features = catboost_selector.get_optimal_features()

        # Plot CatBoost results
        catboost_selector.plot_results()

        # Save CatBoost results
        catboost_selector.save_results_to_csv(catboost_output_dir)

        # CatBoost comprehensive evaluation with SHAP
        catboost_results = catboost_selector.evaluate_final_model(
            X_train, X_test, y_train, y_test,
            feature_names=list(X_train.columns),
            save_shap=True,
            output_dir=catboost_output_dir
        )

        print("\n" + "=" * 80)
        print("CATBOOST FEATURE SELECTION SUMMARY")
        print("=" * 80)
        print(f"Optimal features ({len(catboost_optimal_features)}): {catboost_optimal_features}")
        print(f"Test R²: {catboost_results['test_metrics']['r2']:.6f}")
        print(f"Test MSE: {catboost_results['test_metrics']['rmse'] ** 2:.6f}")
        print(f"Test RMSE: {catboost_results['test_metrics']['rmse']:.6f}")
        print(f"Test MAE: {catboost_results['test_metrics']['mae']:.6f}")
        print(f"Test MAPE: {catboost_results['test_metrics']['mape']:.6f}")
